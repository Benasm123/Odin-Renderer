...
Camera :: struct {
    ...
    rotation : Vec3,
    viewMatrix : bdm.Mat4,
    projectionMatrix : bdm.Mat4,

    up, forward, right : Vec3,
    fov: f32,
    aspect_ratio : f32, // MARKER Consider removing this and fov, and just creating update fov/aspect_ratio function as only need in Projection
    far, near : f32,
...
update_camera_input :: proc(using this : ^Camera, mouse_movement: [2]f32) {
    ...
    rad : Vec3 =  {bdm.to_radians(yaw), bdm.to_radians(pitch), bdm.to_radians(rotation.z)}
}

update_projection_matrix :: proc(using this : ^Camera) {
	q : f32 = 1.0 / (math.tan_f32(bdm.to_radians(fov) / 2))
...
update_view_matrix :: proc(using this : ^Camera) {
	c3 : f32 = math.cos_f32(bdm.to_radians(rotation.z))
	s3 : f32 = math.sin_f32(bdm.to_radians(rotation.z))
	c2 : f32 = math.cos_f32(bdm.to_radians(rotation.y))
	s2 : f32 = math.sin_f32(bdm.to_radians(rotation.y))
	c1 : f32 = math.cos_f32(bdm.to_radians(rotation.x))
	s1 : f32 = math.sin_f32(bdm.to_radians(rotation.x))
	...
	viewMatrix[3, 0] = -bdm.dot3(right, position);
	viewMatrix[0, 1] = up.x;
	viewMatrix[1, 1] = up.y;
	viewMatrix[2, 1] = up.z;
	viewMatrix[3, 1] = -bdm.dot3(up, position);
	viewMatrix[0, 2] = forward.x;
	viewMatrix[1, 2] = forward.y;
	viewMatrix[2, 2] = forward.z;
	viewMatrix[3, 2] = -bdm.dot3(forward, position);
	viewMatrix[0, 3] = 0
	viewMatrix[1, 3] = 0
	viewMatrix[2, 3] = 0
	viewMatrix[3, 3] = 1
